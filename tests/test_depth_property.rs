...
proptest! {
    #[test]
    fn test_depth_property_hugepagevec(size in 0usize..10000) {
    ...
    #[test]
    fn test_depth_property_mmapblock(size in 1usize..10000) {
    ...
    #[test]
    fn test_depth_property_readahead(offset in 0u64..1000, size in 0usize..1000) {
        ...
        prop_assert!(res.is_ok(), "readahead should not error for valid fd: {res:?}");
    }

    #[test]
    fn test_depth_property_mmapcorpus(content in ".*") {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("test.txt");
    ...
    #[test]
    fn test_depth_property_mmapblock_overflow_probes(size in usize::MAX - 4096..=usize::MAX) {
        let block = MmapBlock::new(size);
        prop_assert!(block.is_err(), "Must reject sizes that overflow");
    }

    #[test]
    fn test_depth_property_hugepagevec_overflow_probes(size in usize::MAX / 2..=usize::MAX) {
        let result = HugePageVec::<u64>::new_fallible(size);
        prop_assert!(result.is_err(), "Must reject hugepage sizes that overflow limits");
    }

    #[test]
    fn test_depth_property_binformat_file_header_overflow(version in u64::MAX - 1000..=u64::MAX) {
    ...
    #[test]
    fn test_depth_property_read_section_overflow_probes(size in u64::MAX - 100..=u64::MAX) {
        let mut bytes = Vec::new();
