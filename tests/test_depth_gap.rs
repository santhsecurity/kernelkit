...
#[test]
fn test_depth_gap_hugepagevec_allocation_overflow() -> Result<(), Box<dyn Error>> {
...
#[test]
fn test_depth_gap_mmapcorpus_symlink_loop() -> Result<(), Box<dyn Error>> {
...
#[test]
fn test_depth_gap_mmap_io_error_injection() -> Result<(), Box<dyn Error>> {
    let dir = tempfile::tempdir()?;
...
#[test]
fn test_depth_gap_readahead_io_error() -> Result<(), Box<dyn Error>> {
    ...
    // On Linux readahead is a hint and may be silently ignored; on other
    // platforms it is a documented no-op returning Ok. Either way it must
    // not panic and should return Ok here.
    assert!(res.is_ok(), "readahead on an open deleted file should not error: {res:?}");
    Ok(())
}

#[test]
fn test_depth_gap_oom_injection_hugepagevec() -> Result<(), Box<dyn Error>> {
